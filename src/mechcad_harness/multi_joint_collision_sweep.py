...
class MultiJointCollisionSweepRequestV2(Model):
    ...
    def validate_request(self):
        ...
        if self.schema_version != "multi-joint-collision-sweep-request@2":
            raise ValueError("collision sweep request schema version is not v2")
        if not self.source_assembly_hash.startswith("sha256:"):
            raise ValueError("source assembly hash must be a sha256 identity")
        if self.evaluator_version != MULTI_JOINT_EXACT_COLLISION_SWEEP_V2_VERSION:
            raise ValueError("evaluator version does not match trusted v2 constant")
        if self.model.evaluator_version != MULTI_JOINT_FORWARD_KINEMATICS_V2_VERSION:
        ...
        if self.request_hash == "pending":
            ...
            raise ValueError("request hash does not match canonical v2 request")
        return self
...
class MultiJointCollisionConfigurationResultV2(Model):
    ...
    pair_results: tuple[ExactConstituentPairResultV2, ...] = Field(min_length=1)
    classification: CollisionClassification
    any_interference: bool
    any_touching: bool
    all_positive_clearance: bool
...
class MultiJointDiscreteCollisionSweepService:
    ...
    def _validate_v2_source(
        ...
        object.__setattr__(request, "model", revalidate_v2_kinematic_model(request.model))
        actual_source_hash = assembly_hash(assembly)
        if request.source_assembly_id != assembly.assembly_id:
            raise ValueError("source assembly ID mismatch")
        if request.source_assembly_hash != actual_source_hash:
            raise ValueError("source assembly hash mismatch")
        if request.model.evaluator_version != MULTI_JOINT_FORWARD_KINEMATICS_V2_VERSION:
            raise ValueError("kinematic model evaluator version is not trusted v2")
        if request.model_hash != kinematic_model_hash(request.model):
            raise ValueError("model hash mismatch")
    ...
    @staticmethod
    def _v2_pair_results(
        measurements: object,
        pairs: tuple[tuple[str, str], ...],
        request: MultiJointCollisionSweepRequestV2,
    ) -> tuple[ExactConstituentPairResultV2, ...]:
        try:
            measurements = tuple(measurements)
            measurement_pairs = tuple(
                (measurement[0], measurement[1]) for measurement in measurements
        ...
        for measurement in measurements:
            try:
                first, second, volume, distance = measurement
                volume = float(volume)
                distance = float(distance)
            except (TypeError, ValueError):
                raise ValueError("exact collision measurement is malformed") from None
    ...
    @staticmethod
    def _v2_configuration_classification(
    ...
    def _validate_v2_fk_result(
        ...
        expected_configuration_hash = joint_configuration_hash(configuration)
        if fk_result.evaluator_version != request.model.evaluator_version:
            raise ValueError("forward-kinematics evaluator version mismatch")
        if fk_result.model_id != request.model.model_id:
            raise ValueError("forward-kinematics model ID mismatch")
        if fk_result.source_assembly_hash != source_assembly_hash:
    ...
    def _execute_v2(
        ...
        source_hash = self._validate_v2_source(request, assembly)
        pairs = self.v2_collision_pairs(request)
        ...
        for index, (configuration, fk_result) in enumerate(
            ...
            pair_results = self._v2_pair_results(
                transient_result.measurements, pairs, request
            )
            classification = self._v2_configuration_classification(pair_results)
    ...
    def execute(
        ...
        assembly: CadAssemblyProgram,
    ) -> MultiJointCollisionSweepResult | MultiJointCollisionSweepResultV2:
        if isinstance(request, MultiJointCollisionSweepRequestV2):
            return self._execute_v2(request, assembly)
        request.validate_request()
