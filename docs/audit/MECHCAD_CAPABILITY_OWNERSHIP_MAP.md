...
- `STATUS` uses the audit vocabulary from section 9 of the task:
  ...
  `UNKNOWN`.
- `FIND` references findings in the main report (`F1`…`F17`).
...
| 2 | Immutable revisions | M1 `37f3ff3` | `state/manager.py:create_revision` (WIRED) | — | CD-13 (audit: none) | SINGLE_AUTHORITY |
| 3 | ChangeSet / proposal mutation | M2 `37f3ff3` | `changes/engine.py` (WIRED) | — | — | SINGLE_AUTHORITY |
| 4 | Revision hashing / canonical JSON | M0/M2 `37f3ff3` | `state/hashing.py:canonical_json` (WIRED) | ~20 local `json.dumps` re-implementations with 3 divergent variants | F2 | SEMANTIC_DUPLICATION |
| 5 | Dependency invalidation | M3 `df584f0` | `dependency/graph.py:impact`, `dependency/storage.py` (WIRED) | — | CD-13 (audit: none) | SINGLE_AUTHORITY |
...
| 11 | Backend identity / provenance | M5.5A/B `6cbade0`/`b0d77e1` | `backends/provenance.py:provenance_from_identity` (WIRED) | direct construction in `backends/freecad.py`, `backends/gearworks_cad.py`; `FREECAD_IDENTITY` vs `FREECAD_BACKEND_VERSION` | F5, F12 | POSSIBLE_DUPLICATION |
| 12 | Material / section / warping | M5.5C `4bc2310` | `backends/section_properties.py`, `tools/sections.py` (WIRED) | — | F11 | SINGLE_AUTHORITY (node overload only) |
...
| # | CAPABILITY | ORIG | CURRENT OWNER | OTHER IMPLS | FIND | STATUS |
| --- | --- | --- | --- | --- | --- | --- |
| 18 | Generic CAD / FreeCAD backend | M7A-1/2A `19f77a3` | `backends/freecad.py` (WIRED) | — | F14 | SINGLE_AUTHORITY |
| 19 | Rigid assembly foundation | M7A-2B `19f77a3` | `backends/freecad_assembly.py` (WIRED) | — | — | SINGLE_AUTHORITY |
| 20 | Exact clearance primitive | M7A-2C `19f77a3` | `transient_freecad_measurement.py` (WIRED) | `cad_analysis.py` (TESTS) | F15 | LEGACY_RESIDUE |
...
| 26 | Generated mechanical-part CAD | M13-2 `664ec3b` | `generated_part_cad.py` → `backends/freecad.py` (WIRED) | — | F14 | SINGLE_AUTHORITY (correct extension) |

## D. Motion / kinematics / collision (M10, M13-3P, M13-3)

| # | CAPABILITY | ORIG | CURRENT OWNER | OTHER IMPLS | FIND | STATUS |
| --- | --- | --- | --- | --- | --- | --- |
| 27 | Multi-joint forward kinematics | M10-2 `89b1d75` | `multi_joint_kinematics.py:evaluate` (WIRED) | V1 `/ V2` schemas in same module | F9 | INTENTIONAL_SUPERSESSION |
...
| 30 | Continuous multi-joint path proof | M10-4 `89b1d75` | `multi_joint_continuous_clearance.py` (WIRED) | V1/V2 schemas; one proof engine | F9 | LEGITIMATE_ADAPTER_VARIANTS |
| 31 | Reach-bound derivation | M10-4 `89b1d75` | `multi_joint_continuous_path.py` (WIRED) | V1/V2 | F9 | INTENTIONAL_SUPERSESSION |
...
| 34 | Rigid-body constituent groups (M10 v2) | M13-3P `f3ab0c7` | `multi_joint_pair_scope.py`, v2 schemas (WIRED) | — | F9 | SINGLE_AUTHORITY |
| 35 | Physical pair classification | M12-5 / M13-3 | `models/physical_pair_policy.py:PhysicalPairClassification` (WIRED) | `candidates/canonical_m10.py:CanonicalM10PairClassification` | F8 | POSSIBLE_DUPLICATION |
| 36 | `physical_kinematic_root_hash` | M13-3 `ca294e0` | `models/physical_mechanism.py:35` (WIRED) | `candidates/models.py:58` (WIRED, cross-compared) | F6 | AUTHORITY_CONFLICT (latent) |
| 37 | FK/transform primitives | M7C-1 / M10 / M13-1/2 | `multi_joint_kinematics.py:transform_*` (WIRED) | `models/quaternion.py`, `models/generated_placement.py:compose_poses` | F9 | SEMANTIC_DUPLICATION (low risk) |
...
| 40 | Gmsh mesh generation / MSH parsing | M11-3 `682300b` | `structural/mesh.py` (WIRED) | strict verifier `structural/results.py:_parse_verified_msh` | F12 | INTENTIONAL_INDEPENDENT_VERIFIER (divergent) |
...
| 43 | Structural deck / load lowering | M11-3 `682300b` | `structural/deck.py` (WIRED) | independent verifier in `structural/results.py` | F12 | INTENTIONAL_INDEPENDENT_VERIFIER |
| 44 | Structural Evidence publish/verify | M11-5 `07950cd` | `structural/evidence_service.py` (WIRED) | duplicate artifact-read/verify + `_mesh_input_hash` | F4, F12 | POSSIBLE_DUPLICATION |
...
| 54 | Canonical M10 verification | M12-5 `161986b` | `candidates/canonical_m10.py` (WIRED) | — | F1 | SEMANTIC_DUPLICATION (vs candidate) |
| 55 | M11 handoff eligibility | M12-5 `161986b` | `candidates/m11_handoff.py` (WIRED) | partial re-validation of `models/structural.py` | F12 | POSSIBLE_DUPLICATION |
| 56 | Supplied-component interface authority | M13-1 `f6d8124` | `models/supplied_component_interface.py` (WIRED) | — | — | SINGLE_AUTHORITY |
| 57 | Candidate/canonical multi-joint M10 bridge | M13-3 `ca294e0` | `candidates/multi_joint_m10_bridge.py` (WIRED via service) | single shared lowering core | F9 | SINGLE_AUTHORITY |
...
| --- | --- |
| SINGLE_AUTHORITY | 20 |
| LEGITIMATE_LAYERING | 5 |
| LEGITIMATE_ADAPTER_VARIANTS | 2 |
| INTENTIONAL_SUPERSESSION | 6 |
| INTENTIONAL_INDEPENDENT_VERIFIER | 2 |
| SEMANTIC_DUPLICATION | 11 |
| POSSIBLE_DUPLICATION | 9 |
| AUTHORITY_CONFLICT (latent/overload) | 3 |
| LEGACY_RESIDUE | 6 |
| UNKNOWN | 0 |

Total rows: 60. A row may carry more than one status; the counts above reflect
the primary and secondary classifications per row.
